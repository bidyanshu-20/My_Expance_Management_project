...
import { ArrowDown, TrendingUp as ProfitIcon, PieChart as PieChartIcon, BarChart2, ChevronDown, ChevronUp, DollarSign, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
// this will convert the date into indian standard
...
const Dashboard = () => {
  ...
  // built server-provided recent list
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      if (newTransaction.type === "income") {
        await axios.post("api/income/add", payload, {
          headers: getAuthHeader(),
        });
      }
      else {
        // console.log("I am expanse manager..");
  ...
  return <div className={dashboardStyles.container}>
    <div className={dashboardStyles.headerContainer}>
      <div className={dashboardStyles.headerContent}>
        <div>
          ...
          <p className={dashboardStyles.headerSubtitle}>Track Your income and Expanse</p>
        </div>
    ...
    <div className={dashboardStyles.summaryGrid}>
      <FinancialCard icon={<div className={dashboardStyles.walletIconContainer}>
        ...
        additionalContent={<div className='flex items-center gap-2 text-sm '><span className={dashboardStyles.balanceBadge}>+${Math.round(displayIncome).toLocaleString()}
        </span>
      ...
      } label={`${timeFrameRange.label} Expanses`} value={`${Math.round(displayIncome - displayExpenses).toLocaleString()}`}
        additionalContent={<div className={`mt-2 text-xs flex items-center gap-1 ${expenseChange >= 0 ? trendStyles.positive : trendStyles.negative} `}>
        ...
        additionalContent={
          <div className='mt-2 text-xs text-cyan-600 flex items-center gap-2 '>
            <div className='flex items-center gap-1'>
              ...
              <span className="">{displayIncome > 0 ? Math.round((displaySavings / displayIncome) * 100) : 0} % of Income</span>
            </div>
            {/* //------ */}
    ...
    <AddTransactionModel showModal={showModal} setShowModal={setShowModal} newTransaction={newTransaction} setNewTransaction={setNewTransaction} handleAddTransaction={handleAddTransaction} loading={loading} />
  </div>
}
