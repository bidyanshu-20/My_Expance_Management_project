                ...
                <form action="" onSubmit={(e) => { e.preventDefault(); handleAddTransaction() }}>\
                    <div className={modalStyles.form}>
                        <div>
                            ...
                            <input type="text" value={newTransaction.description} onChange={(e) => setNewTransaction((prev) => ({ ...prev, description: e.target.value, }))} className={colorClass.ring} placeholder={type === "both" ? "Salary, funds, etc." : "Groceries, Rent , etc"} required />
                        </div>
                        <div>
                            <label className={modalStyles.label} htmlFor="">Amount</label>
                            <input type="number" value={newTransaction.amount} onChange={(e) => setNewTransaction((prev) => ({ ...prev, amount: e.target.value, }))} className={colorClass.ring} placeholder="0.00" required />
                        </div>
                        {type === "both" && (
                            <div>
                        ...
                        <div>
                            <label className={modalStyles.label} htmlFor="">Category</label>
                            <select onChange={(e) => setNewTransaction((prev) => ({
                                ...prev,
                                category: e.target.value,
                            }))} className={modalStyles.input(colorClass.ring)} value={newTransaction.category}>{categories.map((cat) => (
                                <option value={cat} key={cat} >{cat}</option>
                            ))}</select>
                        </div>
                        <div>
                            <label className={modalStyles.label} htmlFor="">Date</label>
                            <input type="date" value={newTransaction.date} onChange={(e) => newTransaction
